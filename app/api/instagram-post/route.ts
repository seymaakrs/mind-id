...
export async function POST(request: Request) {
  ...
  try {
    const externalResponse = await fetch(`${baseUrl}/instagram-post`, {
      method: "POST",
    })
    ...
    if (!externalResponse.ok) {
      return NextResponse.json(
        {
          error: "Instagram gonderi paylasim istegi basarisiz.",
    ...
    return NextResponse.json(
      {
        error: "Instagram gonderi servisine ulasilamadi.",
