...
export const DEFAULT_TEAM: TeamMember[] = [
  ...
  { id: "olcay", name: "Olcay", humanRole: "Ajans Müdürü", agentRole: "Orchestrator Agent", quote: "Brief okur, ekibe dagitir", tools: ["Claude 4", "Prompt Router"], color: "#4338ca", repo: "mind-agent", reportsTo: "seyma", status: "active", type: "orchestrator" },
  { id: "defne", name: "Defne", humanRole: "Görsel Tasarimci", agentRole: "Image Agent", quote: "Cizim stüdyosu", tools: ["Gemini 2.5 Flash", "fal.ai"], color: "#065f46", repo: "mind-agent", reportsTo: "olcay", status: "active", type: "agent" },
  { id: "toprak", name: "Toprak", humanRole: "Motion Designer", agentRole: "Video Agent", quote: "Hareket ve ses üretir", tools: ["Veo", "Kling", "HeyGen", "MMAudio"], color: "#065f46", repo: "mind-agent", reportsTo: "olcay", status: "active", type: "agent" },
  ...
  { id: "kaan", name: "Kaan", humanRole: "Stratejist ve Arastirmaci", agentRole: "Analysis Agent", quote: "Veri okur, rapor cikarir", tools: ["Serper.dev", "SEO", "Web scraping"], color: "#92400e", repo: "mind-agent", reportsTo: "olcay", status: "active", type: "agent" },
  { id: "mert", name: "Mert", humanRole: "Satis Akis Yöneticisi", agentRole: "n8n Orchestrator", quote: "Satis pipeline yürütür", tools: ["n8n", "Webhook", "API Bridge"], color: "#6d28d9", repo: "customer_agent", reportsTo: "seyma", status: "building", type: "workflow" },
  { id: "zeynep", name: "Zeynep", humanRole: "Reklam Takipcisi", agentRole: "Meta Lead Agent", quote: "Facebook ve IG lead toplar", tools: ["Meta Ads API", "Webhook"], color: "#374151", repo: "customer_agent", reportsTo: "mert", status: "passive", type: "agent" },
  { id: "emre", name: "Emre", humanRole: "Profesyonel Ag Avcisi", agentRole: "LinkedIn Agent", quote: "B2B lead arar", tools: ["LinkedIn API"], color: "#374151", repo: "customer_agent", reportsTo: "mert", status: "planned", type: "agent" },
  { id: "ayse", name: "Ayse", humanRole: "Veri Zenginlestirme", agentRole: "Clay Local Agent", quote: "Müsteri profilini doldurur", tools: ["Clay", "Clearbit"], color: "#374151", repo: "customer_agent", reportsTo: "mert", status: "planned", type: "agent" },
  { id: "berk", name: "Berk", humanRole: "Mesaj Otomasyonu", agentRole: "Instagram DM Agent", quote: "IG DM otomatik yanitlar", tools: ["Instagram Graph API"], color: "#374151", repo: "customer_agent", reportsTo: "mert", status: "planned", type: "agent" },
  { id: "firebase", name: "Firebase", humanRole: "Sirket Arsivi", agentRole: "Veri Katlani", quote: "Her dosya, her gecmis burada", tools: ["Firestore", "Auth", "Storage"], color: "#b45309", repo: "mindid-nocodb", reportsTo: null, status: "active", type: "data" },
]
...
function buildLayout(
  ...
  members.forEach((m) => {
    if (m.reportsTo && members.find((x) => x.id === m.reportsTo)) g.setEdge(m.reportsTo, m.id)
  })
  ...
  dataIds.forEach((did) => leafIds.slice(0, 3).forEach((lid) => { if (!g.hasEdge(lid, did)) g.setEdge(lid, did) }))

  dagre.layout(g)

  const nodes: Node<NodeData>[] = members.map((m) => {
    const pos = g.node(m.id)
    return { id: m.id, type: "teamMember", position: pos ? { x: pos.x - 95, y: pos.y - 85 } : { x: 0, y: 0 }, data: { member: m, editMode, onEdit, onDelete } }
  })

  const edges: Edge[] = []
  members.forEach((m) => {
    if (m.reportsTo && members.find((x) => x.id === m.reportsTo))
      edges.push({ id: `e-${m.reportsTo}-${m.id}`, source: m.reportsTo, target: m.id, type: "smoothstep", style: { stroke: REPO_COLORS[m.repo] ?? "#6366f1", strokeWidth: 2 } })
  })

  const crossEdges: Array<[string, string, string]> = [["selin", "defne", "isbirligi"], ["selin", "toprak", "isbirligi"], ["mert", "olcay", "SDK koprusu"]]
  crossEdges.forEach(([src, tgt, label]) => {
    if (members.find((m) => m.id === src) && members.find((m) => m.id === tgt))
      edges.push({ id: `collab-${src}-${tgt}`, source: src, target: tgt, type: "straight", style: { stroke: "#64748b", strokeWidth: 1.5, strokeDasharray: "5 5", opacity: 0.55 }, label, labelStyle: { fill: "#94a3b8", fontSize: 10 } })
  })

  dataIds.forEach((did) => members.filter((m) => m.type !== "data" && m.type !== "founder").forEach((m) => {
    edges.push({ id: `data-${m.id}-${did}`, source: m.id, target: did, type: "straight", style: { stroke: "#475569", strokeWidth: 1, strokeDasharray: "3 6", opacity: 0.25 } })
  }))
...
const EMPTY_MEMBER: MemberFormData = { name: "", humanRole: "", agentRole: "", quote: "", tools: [], color: "#6366f1", repo: "mind-agent", reportsTo: "olcay", status: "planned", type: "agent" }

function MemberModal({ title, initial, allMembers, onSave, onClose }: {
  title: string; initial: MemberFormData; allMembers: TeamMember[]; onSave: (data: MemberFormData) => void; onClose: () => void
}) {
  const [form, setForm] = useState<MemberFormData>(initial)
  const [toolsStr, setToolsStr] = useState(initial.tools.join(", "))
  function setField(k: keyof MemberFormData, v: MemberFormData[keyof MemberFormData]) { setForm((f) => ({ ...f, [k]: v })) }
      ...
      <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div><label className={lbl}>Isim *</label><input className={cls} value={form.name} onChange={(e) => setField("name", e.target.value)} placeholder="Defne" /></div>
            <div><label className={lbl}>Renk</label><div className="flex items-center gap-2"><input type="color" value={form.color} onChange={(e) => setField("color", e.target.value)} className="w-10 h-9 rounded-lg cursor-pointer border border-border bg-muted p-0.5" /><span className="text-xs text-muted-foreground font-mono">{form.color}</span></div></div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className={lbl}>Repo</label><select className={cls} value={form.repo} onChange={(e) => setField("repo", e.target.value as TeamMember["repo"])}><option value="mind-agent">mind-agent</option><option value="customer_agent">customer_agent</option><option value="mindid-nocodb">mindid-nocodb</option><option value="mind-id">mind-id</option><option value="root">root</option></select></div>
            <div><label className={lbl}>Tür</label><select className={cls} value={form.type} onChange={(e) => setField("type", e.target.value as AgentType)}><option value="agent">agent</option><option value="orchestrator">orchestrator</option><option value="workflow">workflow</option><option value="data">data</option><option value="integration">integration</option><option value="founder">founder</option></select></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className={lbl}>Raporladigi</label><select className={cls} value={form.reportsTo ?? ""} onChange={(e) => setField("reportsTo", e.target.value || null)}><option value="">— Bagimsiz —</option>{allMembers.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div>
            <div><label className={lbl}>Durum</label><select className={cls} value={form.status} onChange={(e) => setField("status", e.target.value as AgentStatus)}><option value="active">Aktif</option><option value="building">Gelistiriliyor</option><option value="planned">Planlandi</option><option value="passive">Pasif</option></select></div>
          </div>
        </div>
        <div className="flex gap-2 p-4 border-t border-border sticky bottom-0 bg-card">
          ...
          <button onClick={() => { if (!form.name.trim()) return; onSave({ ...form, tools: toolsStr.split(",").map((t) => t.trim()).filter(Boolean) }) }} className="flex-1 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 flex items-center justify-center gap-2">
...
function TeamCanvasInner() {
  const [members, setMembers] = useState<TeamMember[]>(() => {
    ...
    try { const saved = localStorage.getItem(LS_KEY); return saved ? (JSON.parse(saved) as TeamMember[]) : DEFAULT_TEAM } catch { return DEFAULT_TEAM }
  })
  ...
  function handleAddMember(data: MemberFormData) { setMembers((prev) => [...prev, { id: `member-${Date.now()}`, ...data }]); setAddingNew(false) }
  function handleSaveToStorage() { try { localStorage.setItem(LS_KEY, JSON.stringify(members)); setSaveFlash(true); setTimeout(() => setSaveFlash(false), 2000) } catch { /* ignore */ } }
...
export function TeamHierarchyCanvas() {
  return <ReactFlowProvider><TeamCanvasInner /></ReactFlowProvider>
}
